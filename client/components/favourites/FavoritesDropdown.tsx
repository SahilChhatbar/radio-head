...
const StationItem = memo(
  ({
    ...
    const itemBgColor = isCurrentStation
      ? "rgba(255,145,77,0.15)"
      : "transparent";
    const titleTextShadow = isCurrentStation
      ? "0 0 8px rgba(255, 145, 77, 0.6)"
      : "none";
    const titleColor = isCurrentStation ? "#FF914D" : "var(--foreground)";
...
const FavoritesDropdown: React.FC<FavoritesDropdownProps> = ({
  ...
  const hasFavoritesQuery = !!searchQuery;
  const noFavoritesMessage = hasFavoritesQuery
    ? "No favorites match your search"
    : "No favorite stations yet";
  const showHintText = !hasFavoritesQuery;
  const pluralSuffix = favorites.length !== 1 ? "s" : "";
  const isFavoritesListEmpty = filteredFavorites.length === 0;
  ...
  return (
    ...
    >
      ...
      <ScrollArea
        scrollbars="vertical"
        style={{
          height: "clamp(180px, 36.5vh, 370px)",
